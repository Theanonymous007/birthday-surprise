...
(() => {
  ...
  async function saveSurprise() {
    ...
    try {
      ...
      const result = await window.birthdayBackend.create(form);
      const address = new URL(location.href); address.search = ""; address.hash = "";
      address.searchParams.set("surprise", result.id);
      const link = address.href;
  ...
  async function loadGift() {
    ...
    try {
      ...
      const details = await window.birthdayBackend.load(surpriseId);
